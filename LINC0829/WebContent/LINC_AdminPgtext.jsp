				...
				<table class="table table-striped">
					...
					<tbody>
						...
						try {
							...
							tot = rs2.getInt(1);
							rs2.close();
							String sql = "select * from forum LIMIT " + cstart + "," + countList;
						...
						<tr>
							...
							<td><%=s_PostUser%></td>
							<td><%if(Integer.parseInt(is_Delete)==0){%>정상<%}else{%><p style="color: red">삭제</p><%}%></td>
							<td><%=n_ForumCategory%></td>
							<td><form action="LINC_Updateforum.jsp" method="GET" name="Updateforum"><input name="n_PostOrder" type="hidden" value="<%=n_PostOrder%>"><input name="his_Delete" type="hidden" value="<%=is_Delete%>"><input name="cnum" type="hidden" value="<%=Page%>"><%if(Integer.parseInt(is_Delete)==0){%><input type="button" value="삭제" onclick="submit()"><%}else{%><input type="button" value="복구" onclick="submit()"><%}%></form></td>
						</tr>
						<%
							}
