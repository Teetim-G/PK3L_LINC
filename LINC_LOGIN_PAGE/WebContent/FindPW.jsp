...
String findID = request.getParameter("InputID");
String findEmail = request.getParameter("InputEmail");
String sSQL = "update user_stat set s_Password = md5(?) where s_ID = ? and s_EMail = ?;"; 
...
try{
    ...
    out.println("<script>alert('입력하신 이메일로 임시 비밀번호를 전송하였습니다');location.href='LINC_MAIN.jsp';</script>");
%>

<%		
}catch(SQLException e){
	out.println(e.toString());
	out.println("<script>alert('해당 이메일에 해당하는 정보가 없습니다.');history.back();</script>");
}finally{
	if(pstmt!=null)
		pstmt.close();
	if(conn!=null)
